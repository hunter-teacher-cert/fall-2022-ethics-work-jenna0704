...
def get_adj_seats(plane, family_size, col_size):
  if family_size == 2:
    for row in range(len(plane)):
      for col in range(len(plane[row])):
        if plane[row][col] in ['win', 'avail'] and col_size-col>1:
          if plane[row][col+1] in ['win', 'avail']:
            return [[row, col], [row, col+1]]
  elif family_size == 3:
    for row in range(len(plane)):
      for col in range(len(plane[row])):
        if plane[row][col] in ['win', 'avail'] and col_size-col>2:
          if plane[row][col+1] in ['win', 'avail']:
            if plane[row][col+2] in ['win', 'avail']:
              return [[row, col], [row, col+1], [row, col+2]]
  return 'NOT FOUND'


def assign_index(plane, index, name):
  for i in index:
    plane[i[0]][i[1]] = name
  return plane
...
def fill_plane(plane):
    ...
    while total_seats > 1:
        ...
        if r > 30:
            ...
            travelers["u-%d"%u_number] = simulate_travelers(family_size)
    ...
    for name in economy_sold.keys():
        if economy_sold[name] == 1:
            ...
            if check_needs(travelers[name]):
              assigned_seats = get_adj_seats(plane, len(travelers[name]), len(plane[0]))
              if assigned_seats=='NOT FOUND':
                for i in range(economy_sold[name]):
                  plane = seat_economy(plane,economy_sold,name)
              else:
                plane = assign_index(plane, assigned_seats, name)
                sn += [name]
            else:
              for i in range(economy_sold[name]):
                  plane = seat_economy(plane,economy_sold,name)
